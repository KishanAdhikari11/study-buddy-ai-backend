...
@router.post(
    "/generate", # The full path for this endpoint will be /quizzes/generate
    response_model=QuizResponse, # Specifies the expected structure of a successful response
...
async def generate_quiz_endpoint(request: QuizRequest):
    ...
    try:
        ...
        # FastAPI will automatically validate `quiz_result` against `QuizResponse`
        # and serialize it into the JSON response.
        return quiz_result

    except HTTPException:
        # Re-raise HTTPExceptions that were deliberately raised by the code (e.g., 404, 422, 500).
        raise
    except Exception as e:
        # Catch any other unexpected exceptions that might occur during the API call.
