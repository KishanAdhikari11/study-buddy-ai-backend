...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        if file_extension == ".pdf":
            ...
            # For Markdown or text files, just copy them to the output directory
            markdown_path = str(Path(settings.OUTPUT_DIR) / f"{file_id}.md")
            with open(markdown_path, "w", encoding="utf-8") as md_file:
                md_file.write(await file.read().decode('utf-8'))
        if markdown_path:
